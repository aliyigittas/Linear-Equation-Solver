...
for(b=0; control[b] != '\0'; b++){
	
	int length_of_controlb = strlen(control[b]);
	int t = length_of_controlb - 1;
	
	for(j=0; variables_control[j] != '\0'; j++){
		
			if(control[b][t] == variables_control[j]){
			
			printf("variable %c and coefficient is %f\n",variables_control[j] , atof(control[b]) );
		}
			
			else if(control[b][t] == '0'){
				
				printf("constant number is %f\n",atof(control[b]));
			}
